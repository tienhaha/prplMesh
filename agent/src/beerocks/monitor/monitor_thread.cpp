...
void monitor_thread::on_channel_utilization_measurement_period_elapsed()
{
    ...
    /**
     * Compare with previous measurement and if difference has crossed the reporting threshold,
     * then send an AP Metrics Response message to the controller.
     */
    auto radio_node = mon_db.get_radio_node();
    auto &info      = radio_node->ap_metrics_reporting_info();
    if ((UINT8_MAX == info.ap_metrics_channel_utilization_reporting_value) ||
        (abs(channel_utilization - info.ap_metrics_channel_utilization_reporting_value) >
