                                                          ...
                                                          ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    case ieee1905_1::eMessageType::AP_METRICS_RESPONSE_MESSAGE:
        return handle_ap_metrics_response(sd, cmdu_rx);
    default:
        LOG(ERROR) << "Unknown CMDU message type: " << std::hex << int(cmdu_message_type);
        return false;
    }
...
bool slave_thread::handle_ap_metrics_response(Socket &sd, ieee1905_1::CmduMessageRx &cmdu_rx)
{
    const auto mid = cmdu_rx.getMessageId();
    LOG(DEBUG) << "Received AP_METRICS_RESPONSE_MESSAGE, mid=" << std::hex << int(mid);

    auto ap_metrics_response_tlv = cmdu_rx.getClass<wfa_map::tlvApMetric>();
    if (!ap_metrics_response_tlv) {
        LOG(ERROR) << "Failed cmdu_rx.getClass<wfa_map::tlvApMetric>()" << std::hex << int(mid);
        return false;
    }
