...
class PrplwrtDevice:
    ...
    shell_prompt = "_pexpect_prompt_ "
    """We will temporarily use this prompt on the device to make it easier to "expect" it."""
    ...
    def __init__(self, device: str, name: str, image: str, username: str = "root"):
        ...
        self.scriptdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..")
        self.rootdir = os.path.realpath(os.path.join(self.scriptdir))
    ...
    def set_prompt(self, pexp: pexpect):
        """Set the prompt to `shell_prompt` on a pexpect object."""
        pexp.sendline("export PS1='{}'".format(self.shell_prompt))
        pexp.sendline("")
    ...
    def read_remote_version(self) -> List[str]:
        ...
        """
        with pexpect.spawn("ssh {}@{}".format(self.username, self.name)) as shell:
            shell.expect("{}@".format(self.username))
            shell.sendline("cat /etc/prplwrt-version")
            shell.expect("cat /etc/prplwrt-version")
            shell.expect("{}@".format(self.username))
            return shell.before.decode().split()

    def read_local_version(self) -> List[str]:
    ...
    def needs_upgrade(self):
        ...
        with a local one (see `read_local_version).
        """
        local_version = self.read_local_version()
        remote_version = self.read_remote_version()
        diff = difflib.unified_diff(local_version, remote_version,
        ...
        for _ in range(attempts):
            try:
                subprocess.check_output(["ssh", "{}@{}"
                                         .format(self.username, self.name), "true"])
                return True
...
class NetgearRax40(PrplwrtDevice):
    ...
    def upgrade_uboot(self):
        ...
        with serial.Serial(serial_path, self.baudrate) as ser:
            ...
            shell.expect(self.shell_prompt)
            # reboot:
...
def main():
    parser = argparse.ArgumentParser(prog=sys.argv[0],
                                     description="""Update a prplWrt
                                     device, either through u-boot or
                                     using sysupgrade, depending on
                                     the target device.""")
    ...
    parser.add_argument(
        '-i',
        '--image',
        help="Name of the image to use for the upgrade (should be from the artifacts folder).",
        required=True)
    ...
    elif args.device in ["glinet-b1300", "turris-omnia"]:
        dev = Generic(args.device, args.target_name, args.image)
    else:
        raise ValueError("Unknown device: {}".format(args.device))
