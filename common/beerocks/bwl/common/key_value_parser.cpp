...
{
    // Finds first occurrence on string 'str' of
    // <'delim_near'><any_characters except delimiters><'delim_far'>
...
void KeyValueParser::parse_line(std::stringstream &ss_in, std::list<char> delimiter_list,
                                parsed_line_t &parsed_line)
{
    ...
    while (std::getline(ss_in, str_storage, delimiter_list.front())) {
        ...
        if (delimiter_list.size() == 1) {
            ...
            parse_line(ss_out, delimiter_list_out, parsed_line);
        }
    }
}

void KeyValueParser::parse_multiline(std::stringstream &ss_in, std::list<char> delimiter_list,
...
void KeyValueParser::parsed_obj_debug(parsed_line_t &obj)
{
...
void KeyValueParser::parsed_obj_debug(parsed_multiline_t &obj)
{
    LOG(TRACE) << "parsed_obj_debug:";
    std::stringstream ss_obj;
    ss_obj << std::endl << "parsed_obj_debug: " << std::endl;
