...
bool ap_wlan_hal_dwpal::process_dwpal_event(char *buffer, int bufLen, const std::string &opcode)
{
    ...
    case Event::BSS_TM_Response: {
        ...
        if (dwpal_string_to_struct_parse(buffer, bufLen, fieldsToParse,
                                         sizeof(fieldsToParse)) == DWPAL_FAILURE) {
            LOG(ERROR) << "DWPAL parse error ==> Abort";
            return false;
        }
        ...
        std::string mac;
        beerocks::net::network_utils::linux_iface_get_mac(vap_name, mac);
        LOG(DEBUG) << "BTM response source BSSID: " << mac;
        msg->params.source_bssid = beerocks::net::network_utils::mac_from_string(mac);
