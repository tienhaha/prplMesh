                                               ...
                                               const std::string sta_mac, SStaStats &sta_stats)
{
    ...
    // RSSI
    if (!read_param_str("ShortTermRSSIAverage", reply, &tmp_str)) {
    ...
    // SNR
    if (!read_param_str("SNR", reply, &tmp_str)) {
    ...
    if (!read_param_int("LastDataDownlinkRate", reply, tmp_int)) {
        LOG(ERROR) << "Failed reading LastDataDownlinkRate parameter!";
        return false;
    }

    sta_stats.tx_phy_rate_100kb = (uint16_t)(tmp_int / double(100));

    // Last Uplink (RX) Rate
    if (!read_param_int("LastDataUplinkRate", reply, tmp_int)) {
        LOG(ERROR) << "Failed reading LastDataUplinkRate parameter!";
        return false;
    }

    sta_stats.rx_phy_rate_100kb = (uint16_t)(tmp_int / double(100));

    // TX Bytes
    if (!read_param_int("BytesSent", reply, tmp_int)) {
    ...
    // RX Bytes
    if (!read_param_int("BytesReceived", reply, tmp_int)) {
    ...
    // TX Packets
    if (!read_param_int("PacketsSent", reply, tmp_int)) {
    ...
    // RX Packets
    if (!read_param_int("PacketsReceived", reply, tmp_int)) {
    ...
    // Retranmission Count
    if (!read_param_int("RetransCount", reply, tmp_int, true)) {
